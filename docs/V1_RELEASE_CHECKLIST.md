...
| Clean Windows VM validation | Open | 2026-06-28 local package validation and packaged smokes pass for `apps/desktop/release/win-unpacked`; the active blocker is still a healthy clean-VM validation run of the current installer/package because the local host has dev Python and Node on PATH. |
