...
The audit treats verified real LoRA expert training and runtime loading as blocking public V1 release. The relevant standard is also defined in `docs/LORA_CLUSTER_EXPERT_MVP_POLICY.md`: CI scaffold validation is not enough for public release claims, retrieval retains ownership of facts/citations, and LoRA must now be judged primarily on adapter-owned style/terminology/reasoning behavior instead of trying to out-retrieve retrieval.
...
| Live adapter quality benchmark | Failed on 2026-06-20 | `scripts\backend\benchmark-lora-adapter.ps1` wrote `.tmp\lora-adapter-quality-benchmark-2026-06-20.json`; the old strict factual/citation-heavy benchmark scored retrieval `98.33`, adapter `30.0`, delta `-68.33`. This is now treated as an architecture warning, not just a tuning miss. |
| Live adapter quality benchmark, longer generation | Failed on 2026-06-20 | `scripts\backend\benchmark-lora-adapter.ps1 ... -BenchmarkMaxNewTokens 96` wrote `.tmp\lora-adapter-quality-benchmark-2026-06-20-retoken.json`; retrieval `98.33`, adapter `38.67`, delta `-59.66`, so the failure is not just clipped output. |
| Quality-aligned dataset export | Passed on 2026-06-21 | `write_cluster_training_dataset` now emits records for the refocused benchmark contract, including style transfer, terminology consistency, reasoning-pattern reuse, summarization, and refusal. The bounded real retrain attempt from the prior pass also wrote `.tmp\lora-quality-aligned-real-smoke-work\experts\cluster-smoke\adapter-7474a5fa-6a24-49fe-ad89-2b029eb6ea2d\dataset\dataset-manifest.json` with dataset hash `26b3a8f2f491fed1f7bf0aaa5661c9347d79d56974e8008160a2b81e66c32231`, `57` train records, and `15` validation records. |
| Larger-base run matrix | Ready, not yet executed | `scripts\backend\run-lora-size-matrix.ps1` now defines the next evidence loop for `1.5B`, `2B`, and `3B` base-model runs. |
| Benchmark diversity gate | Enforced on 2026-06-21 | The backend now blocks LoRA benchmark-grade training with `insufficient_benchmark_diversity` unless the post-split dataset meets distinct-source/content-hash floors plus per-source share caps. |
| Quality-aligned real retrain | Interrupted on 2026-06-20, then passed bounded CPU retrain on 2026-06-21 | `.tmp\lora-quality-aligned-real-smoke-interrupted-2026-06-20.json` records the earlier real LLaMA Factory command stopping at trainer step `1/6` with `adapter_weights_present=false`. The later bounded CPU run at `.tmp\lora-quality-aligned-cpu512-step1-2026-06-21.json` produced adapter `.tmp\lora-quality-aligned-cpu512-step1-work\experts\cluster-smoke\adapter-ce315f4e-1a28-497f-a65c-9acc014cd9cc` with `adapter_model.safetensors` size `17,640,136` bytes and training dataset hash `9a0f548aa9396dc8aea73ab2affed01c092828805c4ceb11fd51d1ca937b28a0`. |
| Live adapter quality benchmark, quality-aligned adapter | Failed on 2026-06-21 | `.tmp\lora-quality-aligned-cpu512-step1-dataset-match-benchmark-2026-06-21.json` fails closed with `status=dataset_mismatch` because the current docs-derived benchmark dataset hash differs from the adapter training dataset hash. Its raw benchmark still failed at retrieval `98.33`, adapter `61.0`, delta `-37.33`; the embedded same-dataset benchmark in `.tmp\lora-quality-aligned-cpu512-step1-2026-06-21.json` also failed at adapter `49.67`, delta `-48.66`. |
...
The repeatable scaffold path, one real CPU trainer/runtime path, adapter/base pairing proof, and CPU AVX2 proof are now validated, but the public expert blocker remains open because no larger-base adapter run has yet passed the narrowed adapter-owned/shared benchmark contract. The implementation must continue refusing public "trained expert" claims until at least one real `1.5B`/`2B`/`3B` adapter-backed run clears that bar.
