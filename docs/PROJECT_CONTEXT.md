...
| Context Bridge | In progress | `[##########] 99%` | Broader real-client/browser smoke beyond the current desktop quick-save, scoped setup, packaged extension, live HTTP extension-contract smoke, live Chromium extension-popup smoke for the actual popup target and setup/status/upload UI, page/selection/PDF-url capture, downloaded-file upload, screenshot upload, and review flows |
