...
| UI prototype cleanup | In progress | `[#########-] 92%` | Core app routes, Mind workspace, source cards, Apple-like animated onboarding, first startup repair screen, chat composer, prompt-zero chat attachments, Bridge controls, cluster detail, live saved-chat sidebar, diagnostics export UI, runtime/degraded chat states, retriable-generation placeholder UI, and backend-backed actions are in place. Public-V1 UI still needs cluster visual redesign, map fallback/performance, Google OAuth/backend profile finalization, and visual QA. |
...
| Compulsory cluster experts | In progress | `[##--------] 18%` | Expert lifecycle UI/state and job scaffolds exist, hardware status/AVX2 gate is exposed, expert jobs now carry failure code/artifact/hardware-tier fields, adapter artifact records exist, and `train_cluster_adapter` jobs are registered and hardware-gated. Public-V1 language should shift toward cluster memory/context profiles unless real training meets graduation criteria. Real product behavior still needs training implementation, full hardware matrix, pending/failed state UI, structured retry policy, adapter process runner, evaluation, rollback, merge invalidation, and artifact cleanup. |
| Context Bridge | In progress | `[######----] 63%` | Bridge UI, settings/history, HTTP context endpoint, token-gated Bridge access, sanitized allowlists, one-minute permission refresh, manual refresh, semantic retrieval, request logging, extension token/capture scaffold, and first CLI/MCP prototypes exist. Public-V1 blockers remain: core backend auth separation, per-client setup, token lifecycle, MCP notification correctness, app error-code registry, no-active-vault behavior, malformed-client handling, extension UI/onboarding, and real Claude Desktop smoke. |
...
| QA and hardening | In progress | `[#####-----] 52%` | Security pass, dependency audit, compile/build/smoke checks, job architecture documentation, scheduler checkpoint tests, source-page indexing tests, expanded file extraction tests, delete/search cleanup tests, immediate deletion/content clearing tests, duplicate-source tests, reconciliation queue tests, retrieval snapshot tests, chat attachment ingestion/result tests, chatbot-first greeting/general-chat routing regression tests, expanded-analysis foundation tests, reserved-field validation path, coverage-ledger tests, Bridge stale-permission tests, diagnostic-bundle tests, interrupted-generation recovery tests, startup-path smoke, migration baseline test, disk preflight test, embedding-path validation test, local-folder scan/import-history test, extension capture test, expert adapter job scaffold test, vault-lock audit test, hardware-gate smoke test, and this pass's compile/unit/build verification exist. Public-V1 blockers still need Python CVE audit, auth/vault-lock integration tests, deeper migration interruption tests, full startup repair tests, scale/performance benchmarks, map benchmarks, MCP client smoke, diagnostic redaction tests, failure-state tests, and recovery drills. |
...
- Synced-folder/Obsidian scan history is now persisted when tied to a vault. Remaining work: UI for import history, manual refresh, watched refresh, and source reconciliation when files move/delete.
...
- Browser extension local API identity/capture scaffolding now exists. Remaining extension work: actual extension package, onboarding/token pairing UI, capture-current-page/readability extraction, screenshot/PDF handling, and per-client permissions.
...
- LoRA readiness now has artifact records and a `train_cluster_adapter` job scaffold, but no training runner. Treat all adapter output as scaffolded/manual-review until a real runner writes verified artifacts.
