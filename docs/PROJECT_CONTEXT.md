...
| Packaging and installer | In progress | `[######----] 57%` | Windows packaging scaffold, Python runtime staging, one-click NSIS build, icon, unpacked launch smoke, post-install onboarding UI, disk preflight API/UI foundation, non-bundled required embedding setup direction, token-store abstraction seam, and model download scripts exist. Public-V1 blockers remain: install/uninstall smoke, packaged embedding dependency decision, update/migration policy, diagnostic export packaging, and OCR/model staging verification. |
| QA and hardening | In progress | `[######----] 55%` | Security pass, dependency audit, compile/build/smoke checks, job architecture documentation, scheduler checkpoint tests, source-page indexing tests, expanded file extraction tests, delete/search cleanup tests, immediate deletion/content clearing tests, duplicate-source tests, reconciliation queue tests, retrieval snapshot tests, chat attachment ingestion/result tests, chatbot-first greeting/general-chat routing regression tests, expanded-analysis foundation tests, reserved-field validation path, coverage-ledger tests, Bridge stale-permission tests, diagnostic-bundle tests, interrupted-generation recovery tests, startup-path smoke, migration baseline test, disk preflight test, embedding-path validation test, local-folder scan/import-history read/refresh test, extension capture/listing/permission/revoke test, job cancellation test, expert adapter job/artifact read scaffold test, vault-lock audit read test, hardware-gate smoke test, and this pass's compile/unit/build verification exist. Public-V1 blockers still need Python CVE audit, auth/vault-lock integration tests, deeper migration interruption tests, full startup repair tests, scale/performance benchmarks, map benchmarks, MCP client smoke, diagnostic redaction tests, failure-state tests, and recovery drills. |
