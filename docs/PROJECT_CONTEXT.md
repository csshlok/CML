...
| Vault ingestion            | In progress | `[##########] 99%` | Current ingestion handles TXT/Markdown/DOCX/PDF, CSV/JSON/XML/YAML/RTF/logs, common code files, Windows-1252 fallback text decoding, OCRmyPDF+Tesseract scanned-PDF path with direct-Tesseract/PyMuPDF fallback, hardened OCR runtime health checks, registered OCR background job policy/runner, bundled-local-OCR image hooks/readiness UI, OCR runtime staging script, small audio/video metadata, pasted text with null/whitespace validation, static links plus optional browser-rendered dynamic link fallback, URL credential stripping before fetch/storage, drag/drop, local folder import, local folder/Obsidian/synced-folder scan API with visible import history, manual refresh, watched polling refresh jobs, import reconciliation for new/changed/moved/deleted local files, batch outcome counts/failures, Obsidian Markdown frontmatter/wiki-link/embed metadata extraction, browser-extension capture scaffold/listing/revoke/allowlist with same-vault cluster validation, chat attachments as sources, embedding-availability blocking for new indexing paths, disk preflight foundation, basic summaries/tags, queued reindexing, page-aware source/chunk schema, source tombstones, immediate delete content clearing, delete cleanup with retrieval-item tombstoning before page/chunk cleanup, checksum dedupe, 160-file batch regression coverage, deletion graph regression coverage, and live fallback OCR similarity smoke. Remaining public-V1 ingestion risk is external verification only: finish Ghostscript noninteractive staging/OCRmyPDF acceleration and run packaged scanned-PDF/image OCR smoke plus packaged dynamic-link/browser-runtime QA.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| Compulsory cluster experts | In progress | [#####-----] 50%   | Verified LLaMA Factory v0.9.5 installation and trainer availability, configured CML_LORA_TRAINER_COMMAND, validated hardware-gating flow, and updated cluster training dataset export to chat-style records for improved SFT/LoRA compatibility. Expert lifecycle state names, graduation contract API, deterministic dataset generation, train/validation split, dataset/config hashes, trainer process boundary, stdout/stderr capture, required adapter-file verification, metrics/quality-gate evaluation, active adapter selection, rollback support, delete guardrails, artifact version metadata, backend/Desktop API surfaces, trainer dependency status endpoint, and contributor LoRA requirements remain implemented and tested. Remaining public-V1 blockers: execute a real LLaMA Factory training smoke test, generate and validate a real adapter artifact, verify adapter loading in the synthesis runtime, add richer metrics/rollback/failure UI states, expand hardware matrix and failure-code coverage, benchmark adapter quality against retrieval-only responses, and complete runtime QA and packaging validation.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| Packaging and installer    | In progress | `[######----] 64%` | Windows packaging scaffold, Python runtime staging, one-click NSIS build, icon, unpacked launch smoke, post-install onboarding UI, disk preflight API/UI foundation, non-bundled required embedding setup direction, encrypted token-store abstraction with Electron `safeStorage`, model download scripts, OCR runtime staging script, auto-detected local Tesseract-folder staging, qpdf/tessdata staging verification, OCR benchmark command, OCRmyPDF/PyMuPDF packaged-runtime install, packaged loopback CORS allowlist, and OCR runtime status visibility exist. Public-V1 blockers remain: install/uninstall smoke, packaged embedding dependency decision, update/migration policy, diagnostic export packaging, Ghostscript noninteractive staging, OCR/model staging verification, and a fresh package rebuild/smoke when requested.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| QA and hardening           | In progress | `[########--] 86%` | Security pass, dependency audit, compile/build/smoke checks, job architecture documentation, scheduler checkpoint tests, dependent-job blocking/missing-dependency tests, source-page indexing tests, expanded file extraction tests, OCRmyPDF preference/fallback/status tests, Windows-1252 text decoding regression, dynamic-link fallback test, URL credential stripping regression, vault-safety backup test, expanded-analysis evidence-packet test, delete/search cleanup tests, immediate deletion/content clearing tests, retrieval-item tombstone-before-cleanup test, citation tombstone tests, duplicate-source tests, reconciliation queue tests, retrieval snapshot tests, chat attachment ingestion/result tests, chatbot-first greeting/general-chat routing regression tests, expanded-analysis foundation tests, reserved-field validation path, coverage-ledger tests, Bridge stale-permission/no-active-vault/token-rotation/per-client-token tests, MCP notification/error-code tests, diagnostic-bundle and redaction tests, interrupted-generation recovery tests, startup-path smoke, migration baseline/user-version test, disk/model-download preflight tests, concurrent model download guard test, embedding-path/download validation tests, local-folder scan/import-history read/refresh/watch test, extension capture/listing/permission/revoke and same-vault validation tests, job cancellation test, LoRA training contract/dataset/artifact/activation/rollback/dependency-status tests, split contributor requirements coverage tests, vault-lock audit and override-sequence tests, hardware-gate smoke test, Electron token-store plaintext regression, desktop UI build verification, Python `pip check`/`pip-audit` clean state, npm audit clean state, Playwright visual smoke for Home/Settings/Map/cluster-detail/Chat, and full-route Playwright UI interaction audit exist. Public-V1 blockers still need real LLaMA Factory adapter smoke, adapter runtime-load smoke, deeper migration interruption tests, full packaged startup repair tests, scale/performance benchmarks, map benchmarks, real MCP client smoke, disposable-vault destructive UI tests, more failure-state tests, and recovery drills. |
...
- Added bundled-local OCR foundations:
  ...
  - added `scripts/packaging/stage-ocr-runtime.ps1`; installed Tesseract is now auto-detected and staged as a full local runtime folder, and qpdf plus `eng.traineddata` stage successfully
  ...
  - remaining OCR packaging caveat: Ghostscript's installer hangs/cancels noninteractively on this machine, so OCRmyPDF acceleration is not yet package-verified
...
- Completed the OCR/package-readiness ten-step build sprint:
  ...
  - added package-time flags for `-TesseractExePath`, `-SkipGhostscriptInstaller`, and `-AllowPartialOcrRuntime`
  - staged the local app OCR runtime under `backend/bin/ocr` with working Tesseract, `eng.traineddata`, and qpdf
  - verified backend bundled-runtime OCR status without `CML_OCR_BINARY_PATH`: image OCR ready, fallback scanned-PDF OCR ready, OCRmyPDF acceleration incomplete because Ghostscript is missing
