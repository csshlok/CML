...
| --- | --- | --- | --- |
| Product definition | In progress | `[#########-] 95%` | Product PRD, UI PRD, onboarding PRD, project context, architecture doc, model ladder, runtime boundary, model storage decision, installer direction, production-gap notes, job/maintenance architecture, and public-V1 blocker list are documented. Needs final production installer/update policy and explicit public-V1/private-alpha cut line. |
| UI prototype cleanup | In progress | `[#########-] 91%` | Core app routes, Mind workspace, source cards, Apple-like animated onboarding, chat composer, prompt-zero chat attachments, Bridge controls, cluster detail, live saved-chat sidebar, diagnostics export UI, runtime/degraded chat states, retriable-generation placeholder UI, and backend-backed actions are in place. Public-V1 UI still needs startup repair screens, cluster visual redesign, map fallback/performance, Google OAuth/backend profile finalization, and visual QA. |
| Desktop app foundation | In progress | `[########--] 85%` | Electron workspace, Vite dev server, build, file IPC, configurable backend API probing, dev backend process handling, single-instance handling, local backend token handoff, active vault folder config, pre-vault/full-vault backend env wiring, onboarding vault activation, and unpacked packaged launch are working. Public-V1 blockers remain: full startup repair UI, robust runtime process management, vault-lock override UI, and installer smoke. |
| Local backend foundation | In progress | `[#########-] 88%` | SQLite CRUD routes, ingestion endpoints, Bridge settings/token auth, chat streaming/memory status, policy-aware background worker foundation, startup integrity/schema checks, vault ownership lock, optional local API token middleware, Electron token handoff, startup-status writes, pre-vault route guard, startup vector reconciliation queueing, scheduler synthesis gate, runtime state reporting, direct local-LLM generation paths, retriable generation recovery, diagnostic bundle export, expert scaffold, and model status routes work. Public-V1 blockers remain: migration runner, repair failure modes/UI, full auth hardening tests, deletion graph, and broader recovery drills. |
| Vault ingestion | In progress | `[########--] 82%` | Current ingestion handles TXT/Markdown/DOCX/PDF, CSV/JSON/XML/YAML/RTF/logs, common code files, bundled-local-OCR image/scanned-PDF hooks, small audio/video metadata, pasted text, static links, drag/drop, local folder import, chat attachments as sources, embedding-availability blocking for new indexing paths, basic summaries/tags, queued reindexing, page-aware source/chunk schema, source tombstones, delete cleanup, and checksum dedupe. Public-V1 ingestion still needs OCR installer staging/QA, dynamic link extraction, large-cluster scale behavior, disk preflight, deletion graph, and failure/progress policy. |
| Embeddings and clustering | In progress | `[#######---] 72%` | Keyword clustering, chunking, MiniLM-first embedding configuration, dev-only hash fallback, onboarding memory-search selection/test UI, SQLite vector storage with chunk model/index metadata, semantic search, embedding health checks on semantic/reindex paths, source-level broad-query scoring, map search, suggestions, dismissals, merge controls, first incremental vector reconciliation job work, and chat coverage ledgers exist. Public-V1 blockers remain: packaged embedding dependency/model installation, vector-store consistency/compaction, active embedding-index transitions, threshold benchmarks, and merge artifact policy. |
...
| Packaging and installer | In progress | `[#####-----] 54%` | Windows packaging scaffold, Python runtime staging, one-click NSIS build, icon, unpacked launch smoke, post-install onboarding UI, and model download scripts exist. Public-V1 blockers remain: install/uninstall smoke, disk-space checks, packaged embedding runtime/model staging, startup repair flow, update/migration policy, diagnostic export packaging, and OCR/model staging verification. |
| QA and hardening | In progress | `[####------] 44%` | Security pass, dependency audit, compile/build/smoke checks, job architecture documentation, scheduler checkpoint tests, source-page indexing tests, expanded file extraction tests, delete/search cleanup tests, duplicate-source tests, reconciliation queue tests, retrieval snapshot tests, chat attachment ingestion/result tests, chatbot-first greeting/general-chat routing regression tests, expanded-analysis foundation tests, reserved-field validation path, coverage-ledger tests, Bridge stale-permission tests, diagnostic-bundle tests, interrupted-generation recovery tests, startup-path smoke, and this pass's compile/unit/build verification exist. Public-V1 blockers still need Python CVE audit, auth/vault-lock integration tests, migration tests, startup repair tests, disk preflight tests, deletion graph tests, scale/performance benchmarks, map benchmarks, MCP client smoke, diagnostic redaction tests, failure-state tests, and recovery drills. |
...
- Finish the first-run setup from [ONBOARDING_PRD.md](ONBOARDING_PRD.md): add real Google OAuth/backend profile persistence if auth remains in V1, add disk preflight, add first-source import after full-vault readiness, and add startup repair states.
