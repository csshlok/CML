...
- Synced-folder/Obsidian scan history is now persisted when tied to a vault, visible in Settings, manually refreshable, and can reconcile imported sources for new/changed/moved/deleted files with batch outcome counts. Remaining work: always-on watched refresh, Obsidian-specific frontmatter/attachment linking, and packaged scale QA.
