...
| UI prototype cleanup | In progress | `[#########-] 94%` | Core app routes, Mind workspace, source cards, Apple-like animated onboarding, first startup repair screen, chat composer, prompt-zero chat attachments, Bridge controls, cluster detail, adapter artifact visibility, live saved-chat sidebar, diagnostics export UI, Settings hardware/import/extension/job/lock-audit surfaces, runtime/degraded chat states, retriable-generation placeholder UI, and backend-backed actions are in place. Public-V1 UI still needs cluster visual redesign, map fallback/performance, Google OAuth/backend profile finalization, and visual QA. |
| Desktop app foundation | In progress | `[#########-] 88%` | Electron workspace, Vite dev server, build, file IPC, configurable backend API probing, dev backend process handling, single-instance handling, local backend token-store seam, active vault folder config, pre-vault/full-vault backend env wiring, onboarding vault activation, structured startup failure page, and unpacked packaged launch are working. Public-V1 blockers remain: robust runtime process management, vault-lock override UI, and installer smoke. |
| Local backend foundation | In progress | `[#########-] 96%` | SQLite CRUD routes, ingestion endpoints, Bridge settings/token auth, chat streaming/memory status, policy-aware background worker foundation with cancellation, startup integrity/schema checks, schema migration tracking/runner, vault ownership lock with unverifiable-owner protection and audit records/read API, optional local API token middleware, Electron token handoff, startup-status writes/readback, pre-vault route guard, startup vector reconciliation queueing, scheduler synthesis gate, runtime state reporting, direct local-LLM generation paths, retriable generation recovery, diagnostic bundle export, expert scaffold, system preflight/hardware routes, extension capture/listing/permission scaffold, integration history/refresh APIs, and model status routes work. Public-V1 blockers remain: full auth hardening tests, lock override UI, deeper repair flows, deletion graph edge cases, and broader recovery drills. |
| Vault ingestion | In progress | `[#########-] 90%` | Current ingestion handles TXT/Markdown/DOCX/PDF, CSV/JSON/XML/YAML/RTF/logs, common code files, OCRmyPDF+Tesseract scanned-PDF path with direct-Tesseract fallback, bundled-local-OCR image hooks, small audio/video metadata, pasted text, static links, drag/drop, local folder import, local folder/Obsidian/synced-folder scan API with visible import history and manual refresh, browser-extension capture scaffold/listing/revoke/allowlist, chat attachments as sources, embedding-availability blocking for new indexing paths, disk preflight foundation, basic summaries/tags, queued reindexing, page-aware source/chunk schema, source tombstones, immediate delete content clearing, delete cleanup, and checksum dedupe. Public-V1 ingestion still needs OCR installer staging/QA with Ghostscript/qpdf, dynamic link extraction, watched refresh, large-cluster scale behavior, broader deletion graph coverage, and failure/progress policy. |
| Embeddings and clustering | In progress | `[########--] 76%` | Keyword clustering, chunking, non-bundled compulsory embedding setup path, required local embedding model/cache folder validation, local-only sentence-transformers load attempts, MiniLM-first runtime configuration, dev-only hash fallback, onboarding memory-search selection/test UI, SQLite vector storage with chunk model/index metadata, semantic search, embedding health checks on semantic/reindex paths, source-level broad-query scoring, map search, suggestions, dismissals, merge controls, first incremental vector reconciliation job work, and chat coverage ledgers exist. Public-V1 blockers remain: user-facing embedding download/link polish, vector-store consistency/compaction, active embedding-index transitions, threshold benchmarks, and merge artifact policy. |
| Chat and context routing | In progress | `[#########-] 91%` | Global-by-default chat, LLM-first intent routing, chatbot-first greeting/small-talk handling, direct local-runtime chat for general prompts, prompt-zero file attachments, drag/drop attachment staging, attachment-to-source ingestion, stored-attachment confirmation with source names, persisted sessions/messages, pending generation records, retriable startup recovery, combined timeline endpoint, retriable placeholder UI, retrieval snapshots/items, backend chat list/sidebar refresh, streaming, page-aware citations, stale/deleted citation labels, citation source actions, runtime status visibility, source-level coverage-ledger accounting, visible degraded-runtime notes, `expanded_analysis` naming, reserved `complete_analysis` rejection, answer actions, transcript indexing, and local runtime adapter are working. Needs full complete-scope map/reduce answering, token budgets, chat pagination/retention, richer runtime failure UI, and long-running analysis UI. |
| Compulsory cluster experts | In progress | `[####------] 40%` | Expert lifecycle UI/state and job scaffolds exist, hardware status/AVX2 gate is exposed in Settings, expert jobs now carry failure code/artifact/hardware-tier fields, adapter artifact records are listable in cluster detail, and `train_cluster_adapter` jobs are registered and hardware-gated. **Recent contribution (Sk):** implemented cluster dataset generation (`training_dataset.py`), dataset quality evaluation (`training_evaluation.py`), expert profile generation (`expert_profiles.py`), adapter-training workflow integration (`background_jobs.py`), expert artifact creation, hardware capability validation, and training job state transitions. LLaMA Factory has been installed and verified as the intended training backend. Public-V1 language should shift toward cluster memory/context profiles unless real training meets graduation criteria. Remaining work includes actual LoRA training execution, full hardware matrix support, pending/failed state UI, structured retry policy, evaluation automation, rollback handling, merge invalidation, adapter artifact lifecycle management, and cleanup workflows. |

| Context Bridge | In progress | `[#######---] 66%` | Bridge UI, settings/history, HTTP context endpoint, token-gated Bridge access, sanitized allowlists, one-minute permission refresh, manual refresh, semantic retrieval, request logging, extension token/capture scaffold with Settings pairing/listing/revoke and vault allowlists, and first CLI/MCP prototypes exist. Public-V1 blockers remain: core backend auth separation, richer per-client permissions, token rotation history, MCP notification correctness, app error-code registry, no-active-vault behavior, malformed-client handling, full extension package, and real Claude Desktop smoke. |
| Packaging and installer | In progress | `[######----] 58%` | Windows packaging scaffold, Python runtime staging, one-click NSIS build, icon, unpacked launch smoke, post-install onboarding UI, disk preflight API/UI foundation, non-bundled required embedding setup direction, token-store abstraction seam, model download scripts, and OCRmyPDF+Tesseract staging contract exist. Public-V1 blockers remain: install/uninstall smoke, packaged embedding dependency decision, update/migration policy, diagnostic export packaging, and OCR/model staging verification. |
| QA and hardening | In progress | `[######----] 56%` | Security pass, dependency audit, compile/build/smoke checks, job architecture documentation, scheduler checkpoint tests, source-page indexing tests, expanded file extraction tests, OCRmyPDF preference/fallback test, delete/search cleanup tests, immediate deletion/content clearing tests, duplicate-source tests, reconciliation queue tests, retrieval snapshot tests, chat attachment ingestion/result tests, chatbot-first greeting/general-chat routing regression tests, expanded-analysis foundation tests, reserved-field validation path, coverage-ledger tests, Bridge stale-permission tests, diagnostic-bundle tests, interrupted-generation recovery tests, startup-path smoke, migration baseline test, disk preflight test, embedding-path validation test, local-folder scan/import-history read/refresh test, extension capture/listing/permission/revoke test, job cancellation test, expert adapter job/artifact read scaffold test, vault-lock audit read test, hardware-gate smoke test, and this pass's compile/unit/build verification exist. Public-V1 blockers still need Python CVE audit, auth/vault-lock integration tests, deeper migration interruption tests, full startup repair tests, scale/performance benchmarks, map benchmarks, MCP client smoke, diagnostic redaction tests, failure-state tests, and recovery drills. |
=======
| Phase | Status | Progress | Notes |
| --- | --- | --- | --- |
| Product definition | In progress | `[#########-] 97%` | Product PRD, UI PRD, onboarding PRD, connector/extension architecture, project context, architecture doc, model ladder, runtime boundary, model storage decision, installer direction, production-gap notes, job/maintenance architecture, expanded-V1 scope notes, and public-V1 blocker list are documented. Needs final production installer/update policy and explicit public-V1/private-alpha cut line. |
...
| Desktop app foundation | In progress | `[#########-] 92%` | Electron workspace, Vite dev server, build, file IPC, configurable backend API probing, dev backend process handling, single-instance handling, local backend token-store seam, active vault folder config, pre-vault/full-vault backend env wiring, onboarding vault activation, structured startup failure page, Electron vault-lock Open-anyway override action with native confirmation, startup failure copy-details action, embedding model folder picker, and unpacked packaged launch are working. Public-V1 blockers remain: robust runtime process management, full packaged startup repair QA, and installer smoke. |
| Local backend foundation | In progress | `[#########-] 99%` | SQLite CRUD routes, ingestion endpoints, Bridge settings/token auth plus per-client Bridge tokens, chat streaming/memory status, policy-aware background worker foundation with cancellation, startup integrity/schema checks, schema migration tracking/runner, vault ownership lock with unverifiable-owner protection and audit records/read API, optional local API token middleware, Electron token handoff, startup-status writes/readback, pre-vault route guard, startup vector reconciliation queueing, scheduler synthesis gate, runtime state reporting, direct local-LLM generation paths, retriable generation recovery, diagnostic bundle export, expert scaffold, system preflight/hardware/OCR/vault-safety routes, extension capture/listing/permission scaffold, integration history/refresh APIs, model status routes, managed embedding-download status/start/cancel routes, lock override audit records, and Bridge token rotation history work. Public-V1 blockers remain: full auth hardening tests, deeper repair flows, and broader recovery drills. |
| Vault ingestion | In progress | `[#########-] 91%` | Current ingestion handles TXT/Markdown/DOCX/PDF, CSV/JSON/XML/YAML/RTF/logs, common code files, OCRmyPDF+Tesseract scanned-PDF path with direct-Tesseract fallback, OCR runtime health checks, registered OCR background job policy/runner, bundled-local-OCR image hooks, small audio/video metadata, pasted text, static links plus optional browser-rendered dynamic link fallback, drag/drop, local folder import, local folder/Obsidian/synced-folder scan API with visible import history and manual refresh, browser-extension capture scaffold/listing/revoke/allowlist, chat attachments as sources, embedding-availability blocking for new indexing paths, disk preflight foundation, basic summaries/tags, queued reindexing, page-aware source/chunk schema, source tombstones, immediate delete content clearing, delete cleanup, and checksum dedupe. Public-V1 ingestion still needs OCR installer staging/QA with Ghostscript/qpdf, real scanned-PDF/image smoke tests, watched refresh, large-cluster scale behavior, broader deletion graph coverage, and failure/progress policy. |
| Embeddings and clustering | In progress | `[########--] 84%` | Keyword clustering, chunking, non-bundled compulsory embedding setup path, required local embedding model/cache folder validation, Electron folder picker for embedding setup, managed recommended embedding download status/start/cancel API, onboarding/settings download controls, local-only sentence-transformers load attempts, MiniLM-first runtime configuration, dev-only hash fallback, onboarding/settings memory-search selection/test UI, SQLite vector storage with chunk model/index metadata, semantic search, embedding health checks on semantic/reindex paths, source-level broad-query scoring, map search, suggestions, dismissals, merge controls, first incremental vector reconciliation job work, and chat coverage ledgers exist. Public-V1 blockers remain: real download progress bytes, vector-store consistency/compaction, active embedding-index transitions, threshold benchmarks, and merge artifact policy. |
| Chat and context routing | In progress | `[#########-] 92%` | Global-by-default chat, LLM-first intent routing, chatbot-first greeting/small-talk handling, direct local-runtime chat for general prompts, prompt-zero file attachments, drag/drop attachment staging, attachment-to-source ingestion, stored-attachment confirmation with source names, persisted sessions/messages, pending generation records, retriable startup recovery, combined timeline endpoint, retriable placeholder UI, retrieval snapshots/items, backend chat list/sidebar refresh, streaming, page-aware citations, stale/deleted citation labels, citation source actions, runtime status visibility, source-level coverage-ledger accounting, expanded-analysis evidence-packet job records, visible degraded-runtime notes, `expanded_analysis` naming, reserved `complete_analysis` rejection, answer actions, transcript indexing, and local runtime adapter are working. Needs full complete-scope map/reduce answering, token budgets, chat pagination/retention, richer runtime failure UI, and long-running analysis UI. |
| Compulsory cluster experts | In progress | `[##--------] 20%` | Expert lifecycle UI/state and job scaffolds exist, hardware status/AVX2 gate is exposed in Settings, expert jobs now carry failure code/artifact/hardware-tier fields, adapter artifact records are listable in cluster detail, and `train_cluster_adapter` jobs are registered and hardware-gated. Public-V1 language should shift toward cluster memory/context profiles unless real training meets graduation criteria. Real product behavior still needs training implementation, full hardware matrix, pending/failed state UI, structured retry policy, adapter process runner, evaluation, rollback, merge invalidation, and artifact cleanup. |
| Context Bridge | In progress | `[########--] 84%` | Bridge UI, settings/history, HTTP context endpoint, token-gated Bridge access, dedicated per-client Bridge tokens with independent vault/cluster/raw/style/expert permissions, client listing/create/rotate/disable/revoke UI/API, sanitized allowlists, one-minute permission refresh, manual refresh, semantic retrieval, denied-request logging, token rotation history/read API, explicit no-active-vault behavior, MCP notification correctness, first app error-code registry, extension token/capture scaffold with Settings pairing/listing/revoke and vault allowlists, and first CLI/MCP prototypes exist. Public-V1 blockers remain: core backend auth separation, malformed-client hardening, full extension package, and real Claude Desktop smoke. |
| Packaging and installer | In progress | `[######----] 59%` | Windows packaging scaffold, Python runtime staging, one-click NSIS build, icon, unpacked launch smoke, post-install onboarding UI, disk preflight API/UI foundation, non-bundled required embedding setup direction, token-store abstraction seam, model download scripts, OCRmyPDF+Tesseract staging contract, and OCR runtime status visibility exist. Public-V1 blockers remain: install/uninstall smoke, packaged embedding dependency decision, update/migration policy, diagnostic export packaging, and OCR/model staging verification. |
| QA and hardening | In progress | `[#######---] 72%` | Security pass, dependency audit, compile/build/smoke checks, job architecture documentation, scheduler checkpoint tests, source-page indexing tests, expanded file extraction tests, OCRmyPDF preference/fallback/status tests, dynamic-link fallback test, vault-safety backup test, expanded-analysis evidence-packet test, delete/search cleanup tests, immediate deletion/content clearing tests, citation tombstone tests, duplicate-source tests, reconciliation queue tests, retrieval snapshot tests, chat attachment ingestion/result tests, chatbot-first greeting/general-chat routing regression tests, expanded-analysis foundation tests, reserved-field validation path, coverage-ledger tests, Bridge stale-permission/no-active-vault/token-rotation/per-client-token tests, MCP notification/error-code tests, diagnostic-bundle tests, interrupted-generation recovery tests, startup-path smoke, migration baseline test, disk preflight test, embedding-path/download validation tests, local-folder scan/import-history read/refresh test, extension capture/listing/permission/revoke test, job cancellation test, expert adapter job/artifact read scaffold test, vault-lock audit read test, hardware-gate smoke test, desktop UI build verification, Playwright visual smoke for Home/Settings/Map/cluster-detail/Chat, and full-route Playwright UI interaction audit exist. Public-V1 blockers still need Python CVE audit, auth/vault-lock integration tests, deeper migration interruption tests, full startup repair tests, scale/performance benchmarks, map benchmarks, real MCP client smoke, disposable-vault destructive UI tests, diagnostic redaction tests, failure-state tests, and recovery drills. |

> > > > > > > 0080411c751f29203f90cb5f7cac13903ccdccf5
...
- Because LoRA is now required in the expanded V1 scope, implement the smallest honest adapter-training spike next: hardware matrix, AVX2/unknown handling, training job process wrapper, adapter artifact table, structured failure codes, and a quality comparison against retrieval-only answers before any user-facing expert claim.
- Adapter artifact storage and training job registration now exist. Remaining LoRA work: process runner, training data export, artifact file layout, evaluation harness, rollback, cleanup, UI states, and quality comparison.
...
- Browser extension local API identity/capture scaffolding now exists; Settings can create and revoke a local extension token, and clients can be restricted by allowed vault IDs. Remaining extension work: actual extension package, safer pairing flow, capture-current-page/readability extraction, screenshot/PDF handling, and richer per-client permissions.
...
- Continue Bridge permission hardening. Permissions now prune stale vault/cluster IDs and token rotations are tracked, but per-client permissions, malformed-client hardening, and real MCP client smoke remain.
...
- Continue the cross-system failure coordination contract before implementing the remaining runtime/auth/expert/write-lock hardening:
  ...
  - runtime crash and backend restart during indexing still need state transition diagrams; vault lock contention now has an Electron Open-anyway path plus backend override audit, but still needs packaged visual QA
...
- Vault lock contention now has a first Electron override path: the startup repair screen can trigger a one-shot `CML_VAULT_LOCK_OVERRIDE=open_anyway` backend restart, and the backend writes an explicit `override_open_anyway` audit row. This still needs packaged visual QA and clearer user-facing corruption copy before public release.
...
- Public V1 gate: auth hardening, vault-lock override UX, packaged startup repair QA, vault data path correctness, embedding setup polish, deletion graph edge cases, scheduler synthesis gate, diagnostic redaction tests, and MCP smoke are blockers. If incomplete, the July build is private alpha/demo only.
...
- Token storage now has an Electron interface seam, but Windows DPAPI is not implemented yet. The current implementation remains a restricted local file fallback.
- Vault lock reclaim is now safer for unverifiable live processes, but the user-facing Cancel/Open-anyway override and two-phase audit log are still not implemented.
- Vault lock audit rows now exist for backend lock events. The missing product piece is still the Electron-facing Cancel/Open-anyway override flow and two-phase user-choice audit.
- Embedding setup now requires a local model/cache folder and uses local-files-only loading. The remaining UX gap is a clean model download/link flow with progress, cancellation, and clear destination paths.
...
- LoRA readiness now has a hardware status gate and expert job metadata, not a training pipeline. Real adapter work still needs a process runner, artifact schema, evaluation harness, rollback, and cleanup.
- LoRA readiness now has artifact records, cluster-detail artifact visibility, and a `train_cluster_adapter` job scaffold, but no training runner. Treat all adapter output as scaffolded/manual-review until a real runner writes verified artifacts.
