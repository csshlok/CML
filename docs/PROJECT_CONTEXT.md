...
- Additional validated follow-up: the same model-recommender helper scripts now accept `-ApiToken`, default it from `CML_API_TOKEN`, and send `x-cml-api-token` on API calls so they work with the default authenticated dev backend. Validation passed: `.\.venv\Scripts\python.exe -m pytest -q backend\tests\test_model_recommender.py` (`26 passed`), PowerShell parser check for the five touched helper scripts (passed), targeted grep confirmed each helper sets `x-cml-api-token`, `.\.venv\Scripts\python.exe -m compileall -q backend\tests\test_model_recommender.py` (passed), and `.\.venv\Scripts\python.exe -m ruff check backend\tests\test_model_recommender.py` (passed).
- Initial validated fix for this pass: `docs/WORKING_COMMANDS.md` now keeps all version-bump steps under the Version Bump Procedure instead of splitting desktop/backend version updates under Benchmark Reports. Validation passed: top-level heading scan still shows sections `1` through `10` in order, and an ordering check confirmed root, desktop, backend/lockfile, and version-rule steps appear before `## 6. Benchmark Reports`.
- Final validated cleanup for this pass: `docs/WORKING_COMMANDS.md` now uses contributor-local placeholders for LoRA model roots and adapter paths instead of machine-specific drive/profile examples. Validation passed: targeted grep found no stale absolute-path examples in current runbook/current-context docs; top-level runbook heading scan still showed sections `1` through `10`; `.\.venv\Scripts\python.exe -m pytest -q backend\tests` (`544 passed, 3 skipped`); `npm run lint` (`40 passed`); `npm run build` (passed); `node --test apps\browser-extension\tests\*.test.cjs` (`20 passed`); `.\.venv\Scripts\python.exe -m compileall -q backend\app scripts\backend backend\tests` (passed); `npx tsc --project apps\desktop\tsconfig.json --noEmit` (passed); `.\.venv\Scripts\python.exe -m ruff check backend scripts` (passed); `npm run security:renderer` (passed); and `npm run security:package` (passed).
...
| Model recommendation | In progress | `[#########-] 88%` | Hardware-aware chat/expert distinction exists, wording now reflects expert compression, and helper scripts now target the current backend port with optional `CML_API_TOKEN` auth; broader runtime/setup verification and final release-gate proof are still pending. |
