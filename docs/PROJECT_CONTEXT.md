...
| UI prototype cleanup | In progress | `[##########] 98%` | Core app routes, generated-reference Vault shell/sidebar, Home/Mind workspace, Sources ledger, Clusters list, specified Map card graph, specified cluster-detail workspace, prompt-zero Chat, Settings/Profile, Tasks, Timeline/Activity, refreshed animated onboarding, Bridge controls, adapter artifact visibility, live saved-chat sidebar, diagnostics export UI, runtime/degraded chat states, retriable-generation placeholder UI, bottom user tab/footer, and backend-first/mock-fallback actions are in place. Public-V1 UI still needs exact-match polish on Chat detail, Bridge, onboarding packaged flow, dark-mode QA, and broader responsive/visual QA. |
...
| QA and hardening | In progress | `[######----] 61%` | Security pass, dependency audit, compile/build/smoke checks, job architecture documentation, scheduler checkpoint tests, source-page indexing tests, expanded file extraction tests, OCRmyPDF preference/fallback/status tests, dynamic-link fallback test, vault-safety backup test, expanded-analysis evidence-packet test, delete/search cleanup tests, immediate deletion/content clearing tests, duplicate-source tests, reconciliation queue tests, retrieval snapshot tests, chat attachment ingestion/result tests, chatbot-first greeting/general-chat routing regression tests, expanded-analysis foundation tests, reserved-field validation path, coverage-ledger tests, Bridge stale-permission tests, diagnostic-bundle tests, interrupted-generation recovery tests, startup-path smoke, migration baseline test, disk preflight test, embedding-path validation test, local-folder scan/import-history read/refresh test, extension capture/listing/permission/revoke test, job cancellation test, expert adapter job/artifact read scaffold test, vault-lock audit read test, hardware-gate smoke test, desktop UI build verification, and Playwright visual smoke for Home/Settings/Map/cluster-detail/Chat exist. Public-V1 blockers still need Python CVE audit, auth/vault-lock integration tests, deeper migration interruption tests, full startup repair tests, scale/performance benchmarks, map benchmarks, MCP client smoke, diagnostic redaction tests, failure-state tests, and recovery drills. |
...
- Preserve backend-first data loading with mock fallback for all redesigned pages so the reference UI stays production-shaped rather than static-only.
