...
- Browser extension local API identity/capture scaffolding now exists; Settings can create and revoke a local extension token, and clients can be restricted by allowed vault IDs. Remaining extension work: actual extension package, safer pairing flow, token rotation history, capture-current-page/readability extraction, screenshot/PDF handling, and richer per-client permissions.
