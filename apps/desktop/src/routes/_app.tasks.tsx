...
type TaskFilter = "active" | "attention" | "history";
type ProjectTask = { project: ProjectRecord; run: ProjectIndexRunRecord };
const taskFilters: TaskFilter[] = ["active", "attention", "history"];
...
function TasksView() {
  ...
  const rows = useMemo(() => {
    ...
    return allJobs
      ...
      .filter(
        (job) =>
          !normalized ||
          `${job.job_type} ${job.status} ${job.write_scope ?? ""}`
            .toLowerCase()
            .includes(normalized),
      );
  ...
  const visibleProjectTasks = useMemo(() => {
    const normalized = query.trim().toLowerCase();
    return projectTasks
      .filter(({ run }) => matchesProjectFilter(run, filter))
      .filter(
        ({ project, run }) =>
          !normalized ||
          `${project.name} ${run.phase} ${run.status} ${run.trigger_source}`
            .toLowerCase()
            .includes(normalized),
      );
  ...
  async function setSelectedPaused(paused: boolean) {
    if (!selected) return;
    try {
      const next = paused ? await pauseJob(selected.id) : await resumeJob(selected.id);
      setSelected(next);
  ...
  const activeJob = selected ? (allJobs.find((job) => job.id === selected.id) ?? selected) : null;
  ...
  return (
    ...
    >
      <main className="mx-auto w-full max-w-[1280px] min-w-0 px-4 py-6 sm:px-6 lg:px-8 lg:py-8 xl:overflow-y-auto">
        ...
        <nav aria-label="Task status" className="mt-5 flex gap-1 border-b border-border">
          {taskFilters.map((item) => (
            <button
              key={item}
              type="button"
              ...
              className={`flex min-h-10 items-center gap-2 border-b-2 px-3 text-sm transition-colors ${
                filter === item
                  ...
                  <details key={run.id} className="group px-4 py-3">
                    <summary className="flex cursor-pointer list-none flex-wrap items-center justify-between gap-3 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
                      <div className="min-w-0">
                        ...
                        <div className="mt-1 text-xs text-muted-foreground">
                          {run.phase.replaceAll("_", " ")} · {complete.toLocaleString()} /{" "}
                          {total.toLocaleString()} · {formatDate(run.updated_at)}
                        </div>
                      </div>
                    ...
                    <div className="mt-4 grid gap-4 border-t border-border pt-4 text-xs text-muted-foreground sm:grid-cols-2">
                      <div className="space-y-2">
                        <Meta
                          label="Triggered by"
                          value={run.trigger_source.replaceAll("_", " ")}
                        />
                        <Meta
                          label="Heartbeat"
                          value={run.heartbeat_at ? formatDate(run.heartbeat_at) : "not reported"}
                        />
                        <Meta label="Skipped" value={run.skipped_count.toLocaleString()} />
                        <Meta label="Failed" value={run.failed_count.toLocaleString()} />
                      </div>
                      <div>
                        <div className="font-medium text-foreground">Persisted phases</div>
                        <ul className="mt-2 space-y-1">
                          {Object.keys(detail)
                            .filter((key) => key.endsWith("_job_id"))
                            .map((key) => (
                              <li key={key}>{key.replace("_job_id", "").replaceAll("_", " ")}</li>
                            ))}
                          {Object.keys(detail).filter((key) => key.endsWith("_job_id")).length ===
                            0 && <li>No child phase IDs were reported.</li>}
                        </ul>
                      </div>
                    </div>
                    <div className="mt-4 flex flex-wrap gap-2">
                      ...
                      {["queued", "running"].includes(run.status) && (
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => void cancelProjectRun(project.id).then(load)}
                        >
                          Cancel
                        </Button>
                      )}
                      {["failed", "partial", "cancelled"].includes(run.status) && (
                        <Button
                          ...
                          onClick={() =>
                            void reindexProject(
                              project.id,
                              run.phase.includes("retrieval") ? "retrieval" : "structure",
                            ).then(load)
                          }
                        >
                          Retry failed layer
          ...
          <div className="flex flex-col gap-3 border-b border-border px-4 py-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                {rows.length} {rows.length === 1 ? "job" : "jobs"} in{" "}
                {labelForFilter(filter).toLowerCase()}
              </p>
            </div>
                  ...
                  >
                    ...
                    <span className="min-w-0">
                      ...
                      <span className="mt-1 block truncate text-xs text-muted-foreground">
                        {job.status_detail || job.dedupe_key || job.id}
                      </span>
                    </span>
                    <span className="truncate text-muted-foreground">
                      {job.write_scope ?? "none"}
                    </span>
                    ...
                    <ChevronRight
                      className="h-4 w-4 justify-self-end text-muted-foreground"
                      aria-hidden="true"
                    />
                  </button>
                ))}
      ...
      {activeJob ? (
        <aside className="min-w-0 border-t border-border bg-card px-4 py-6 sm:px-6 xl:overflow-y-auto xl:border-l xl:border-t-0 xl:py-8">
          ...
          </div>
          {
            <div className="mt-5">
              ...
              <h3 className="mt-6 break-words text-[15px] font-semibold leading-snug">
                {jobTitle(activeJob.job_type)}
              </h3>
              <p className="mt-3 break-words text-sm leading-6 text-muted-foreground">
                {activeJob.status_detail ||
                  activeJob.last_error ||
                  "No additional job detail was reported."}
              </p>
              <dl className="mt-7 divide-y divide-border border-y border-border text-sm">
                ...
                <Meta
                  label="Attempts"
                  value={`${activeJob.attempts} / ${activeJob.max_attempts}`}
                />
                <Meta
                  label="Timeout"
                  value={activeJob.timeout_seconds ? `${activeJob.timeout_seconds}s` : "none"}
                />
                <Meta
                  label="Started"
                  value={activeJob.started_at ? formatDate(activeJob.started_at) : "not started"}
                />
                {activeJob.diagnostic_id ? (
                  <Meta label="Reference" value={activeJob.diagnostic_id} />
                ) : null}
              </dl>
...
function matchesFilter(job: AppJobRecord, filter: TaskFilter) {
  ...
  if (filter === "attention") {
    return ["failed", "partial_success", "manual_review"].includes(job.status);
  }
  return ["succeeded", "cancelled"].includes(job.status) && job.user_visible !== 0;
}
...
function emptyDescription(filter: TaskFilter, hasQuery: boolean) {
  ...
  return {
    ...
    attention: "Failed or partially completed work will appear here with recovery details.",
    history: "Finished and cancelled tasks will appear here.",
  }[filter];
}
...
function statusIcon(status: string) {
  ...
  if (status === "failed" || status === "partial_success" || status === "manual_review")
    return <AlertTriangle className="h-4 w-4 text-destructive" />;
  if (status === "succeeded")
    return <CheckCircle2 className="h-4 w-4 text-[var(--status-ready)]" />;
...
function formatEstimate(job: AppJobRecord) {
  if (job.estimated_remaining_seconds != null)
    return `${Math.max(0, Math.round(job.estimated_remaining_seconds))}s`;
  if (job.elapsed_seconds != null && job.status === "running")
    return `${Math.round(job.elapsed_seconds)}s elapsed`;
  return "-";
}
...
function matchesProjectFilter(run: ProjectIndexRunRecord, filter: TaskFilter) {
  ...
  if (filter === "attention") return ["failed", "partial", "interrupted"].includes(run.status);
