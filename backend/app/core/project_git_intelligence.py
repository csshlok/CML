...
def refresh_git_intelligence(project_id: str, *, max_commits: int = 1000, max_pairs: int = 5000) -> dict:
    ...
    with connect() as conn:
        ...
        owning_snapshot = project.get("active_manifest_snapshot_id") or project.get("active_snapshot_id")
        indexed_paths = {
            _normal_path(str(item["relative_path"]))
            for item in conn.execute("SELECT relative_path FROM project_sources WHERE project_id = ?", (project_id,))
        }

    repository = _git(root, "rev-parse", "--is-inside-work-tree")
    if repository.returncode != 0 or repository.stdout.strip() != "true":
        return _store_git_snapshot(project, owning_snapshot, now, indexed_paths=indexed_paths,
    ...
    log = _git(
        root, "log", "--no-merges", f"-n{limit}", "--date=iso-strict",
        "--format=%x1e%H%x1f%an%x1f%aI%x1f%s", "--numstat",
    )
    ...
    for commit in commits:
        ...
        for path, additions, deletions in commit["files"]:
            signal = file_signals.setdefault(path, {
                "additions": 0, "deletions": 0, "commit_count": 0, "bugfix_commit_count": 0,
                "last_commit_id": commit["id"], "last_commit_at": commit["at"],
    ...
    for path, signal in file_signals.items():
        ...
        signal["ownership"] = [
            ...
            for author, count in sorted(authors[path].items(), key=lambda item: (-item[1], item[0].casefold()))
        ]
    ...
    recent = [{key: commit[key] for key in ("id", "author", "at", "subject")} for commit in commits[:20]]
    return _store_git_snapshot(
        project, owning_snapshot, now, indexed_paths=indexed_paths, head=head, branch=branch,
        shallow=shallow, history_truncated=history_truncated, total_commits=total_commits,
        live_state=live_state, recent_commits=recent, file_signals=file_signals,
...
def get_project_repository_state(project_id: str) -> dict:
    with connect() as conn:
        ...
        indexed_paths = {
            ...
            for value in conn.execute("SELECT relative_path FROM project_sources WHERE project_id = ?", (project_id,))
        }
        row = conn.execute(
            "SELECT * FROM project_git_snapshots WHERE project_id = ? ORDER BY generated_at DESC LIMIT 1", (project_id,)
        ).fetchone()
        if row is None:
            return {"status": "unavailable", "unknown_reason": "Git intelligence has not been refreshed."}
        item = dict_from_row(row)
        signals = [dict_from_row(value) for value in conn.execute(
            "SELECT * FROM project_git_file_signals WHERE git_snapshot_id = ? ORDER BY commit_count DESC, relative_path LIMIT 200",
            (item["id"],),
        )]
        cochange = [dict_from_row(value) for value in conn.execute(
...
def _store_git_snapshot(project: dict, owning_snapshot: str | None, now: str, *, indexed_paths: set[str],
                        head: str = "", branch: str = "", shallow: bool = False,
                        history_truncated: bool = False, total_commits: int = 0,
                        live_state: dict | None = None, recent_commits: list | None = None,
                        file_signals: dict | None = None, pair_counts: list | None = None,
    ...
    live_state = live_state or {"files": [], "counts": {}, "represented_file_count": len(indexed_paths)}
    with connect() as conn:
        ...
        conn.execute(
            ...
            (snapshot_id, project["id"], owning_snapshot, project.get("indexed_commit"), head or None, branch,
             int(bool(head)), int(history_truncated), int(shallow), total_commits, _json(live_state),
             _json(recent_commits or []), error_detail, now),
        )
        for path, signal in sorted(file_signals.items()):
            conn.execute(
                """INSERT INTO project_git_file_signals VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (project["id"], snapshot_id, path, signal["additions"], signal["deletions"],
                 signal["commit_count"], signal["bugfix_commit_count"], signal["last_commit_id"],
                 signal["last_commit_at"], signal["last_commit_subject"], _json(signal["ownership"]),
                 int(history_truncated)),
            )
        for (source, target), count in pair_counts or []:
            conn.execute(
                "INSERT INTO project_cochange_edges VALUES (?, ?, ?, ?, ?, ?, ?)",
                (project["id"], snapshot_id, source, target, count, "heuristic",
                 "historical_cochange_not_dependency"),
            )
        summary = {
            "status": "ready" if head else "unavailable", "head_commit": head or None, "branch": branch or None,
            "indexed_relation": live_state.get("indexed_relation", "unknown"), "history_available": bool(head),
            "history_truncated": history_truncated, "shallow_history": shallow,
            "commit_count": total_commits, "changed_file_count": len(live_state.get("files", [])),
        ...
        if intelligence:
            ...
            layers["repository_signals"] = {
                "status": "ready" if head else "unavailable", "version": GIT_INTELLIGENCE_VERSION,
                "generated_at": now, "truncated": history_truncated,
...
def _parse_log(text: str) -> list[dict]:
    ...
    for block in text.split("\x1e"):
        ...
        commits.append({"id": header[0], "author": header[1], "at": header[2], "subject": header[3][:500], "files": files})
    return commits
...
def _live_state(root: Path, indexed_paths: set[str]) -> dict:
    ...
    while index < len(entries):
        ...
        if xy[0] in "RC" and index < len(entries):
            ...
            index += 1
        states = []
        if xy == "??": states = ["untracked"]
        else:
            if xy[0] not in " ?": states.append("staged")
            if xy[1] not in " ?": states.append("unstaged")
            if "R" in xy: states.append("renamed")
            if "D" in xy: states.append("deleted")
        for state in states: counts[state] += 1
        files.append({"relative_path": path, "status": states, "represented_in_index": path in indexed_paths,
                      "active_content_current": False})
    return {"files": files, "counts": dict(sorted(counts.items())), "represented_file_count": len(indexed_paths),
            "truncated": False}


def _commit_relation(root: Path, indexed: object, head: str) -> str:
    indexed_value = str(indexed or "")
    if not indexed_value or not head: return "unknown"
    if indexed_value == head: return "equal"
    if _git(root, "merge-base", "--is-ancestor", indexed_value, head).returncode == 0: return "behind"
    if _git(root, "merge-base", "--is-ancestor", head, indexed_value).returncode == 0: return "ahead"
    return "diverged"
