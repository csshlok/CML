...
EXTRACTOR_VERSION = f"odin-manifest-v1+{STRUCTURE_EXTRACTOR_VERSION}"
...
def sync_project(project_id: str, *, trigger_source: str = "manual") -> dict:
    ...
    with connect() as conn:
        conn.execute(
            ...
            """,
            (run_id, project_id, trigger_source, now, now, now),
        )
        conn.execute(
            "UPDATE projects SET status = 'indexing', structure_status = 'processing', updated_at = ? WHERE id = ?",
            (now, project_id),
        )
    try:
        ...
        with connect() as conn:
            ...
            conn.execute(
                "UPDATE projects SET status = 'issue', structure_status = 'issue', updated_at = ? WHERE id = ?",
                (utc_now(), project_id),
            )
        raise
    return {"project": get_project(project_id), "run": get_project_run(run_id), "snapshot_id": snapshot_id}


def reindex_project(project_id: str, *, layer: str) -> dict:
    normalized = layer.strip().lower()
    if normalized in {"structure", "full"}:
        return sync_project(project_id, trigger_source=f"reindex_{normalized}")
    if normalized not in {"retrieval", "interpretation"}:
    ...
    with connect() as conn:
        ...
        source_rows = conn.execute(
            "SELECT source_id FROM project_sources WHERE project_id = ? ORDER BY relative_path",
            (project_id,),
        ).fetchall()
        for row in source_rows:
            enqueue_job(
                ...
                dedupe_key=f"reindex-source:{row['source_id']}",
            )
            queued += 1
        conn.execute(
            "UPDATE projects SET retrieval_status = 'processing', updated_at = ? WHERE id = ?",
            (utc_now(), project_id),
        )
    return {"project": get_project(project_id), "queued_jobs": queued, "layer": normalized}
...
def cancel_project_run(project_id: str) -> dict:
    with connect() as conn:
        ...
        conn.execute(
            """
            UPDATE project_index_runs
            SET cancellation_requested = 1, status = 'cancelled', finished_at = ?, updated_at = ?
            WHERE id = ?
            """,
            (now, now, row["id"]),
        )
...
def discover_project(root: Path) -> DiscoveryResult:
    ...
    with connect() as conn:
        ...
        conn.execute(
            ...
            UPDATE projects SET repository_kind = ?, git_remote_fingerprint = ?, default_branch = ?, indexed_commit = ?,
                ...
                active_snapshot_id = ?, brief = ?, languages_json = ?, workspace_count = ?,
                entrypoints_json = ?, updated_at = ?
            WHERE id = ?
            """,
            (
                repository_kind,
        ...
        conn.execute(
            ...
            UPDATE project_index_runs SET snapshot_id = ?, status = 'succeeded', phase = 'activated',
                ...
                detail_json = ?, finished_at = ?, updated_at = ? WHERE id = ?
            """,
            (
                snapshot_id,
                len(discovery.files),
