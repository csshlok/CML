...
_PURPOSE_PATHS = ("readme.md", "readme.mdx", "readme.txt", "readme", "package.json", "pyproject.toml")
...
def build_project_intelligence(
    ...
    *,
    ...
    file_count = int(indexed_file_count) if indexed_file_count is not None else len(getattr(discovery, "files", []) or [])
    identity = {
    ...
    interpretation = {
        ...
        "primary_evidence_ids": [purpose_candidates[0]["evidence_id"]] if purpose_candidates else [],
    }
    purpose_unknown = None if purpose_candidates else {
    ...
    architecture_status = structure_status if structure_status in {"ready", "partial", "stale", "unavailable", "failed"} else "waiting"
    layers = {
        "identity": _layer("ready" if purpose_candidates else "partial", PURPOSE_EXTRACTOR_VERSION, now, purpose_unknown),
        "architecture": _layer(architecture_status, structure_extractor_version, now),
        "repository_signals": _layer("unavailable", "not-built", None, {
            "code": "not_built",
            "detail": "Git behavioral intelligence has not been built for this project.",
        }),
        "decisions": _layer("unavailable", "not-built", None, {
            "code": "not_built",
    ...
    freshness = {
        ...
        "indexed_commit": indexed_commit if indexed_commit is not None else project.get("indexed_commit"),
    ...
    conn.execute(
        ...
        (
            ...
            *(_json(payloads[key]) for key in (
                "identity_json", "architecture_json", "repository_signals_json",
                "decisions_json", "interpretation_json", "freshness_json", "layer_states_json",
            )),
            now,
            now,
    ...
    for item in evidence:
        conn.execute(
            ...
            """,
            (
                item["id"], intelligence_id, project["id"], item["source_type"], item["source_id"],
                item["relative_path"], owning_snapshot_id, item["start_line"], item["end_line"],
                item["extraction_method"], item["extractor_version"], item["confidence_class"],
                item["excerpt_hash"], item["verification_state"], item["label"], now,
            ),
        )
...
def get_project_intelligence(project_id: str) -> dict:
    with connect() as conn:
        ...
        active_manifest = project.get("active_manifest_snapshot_id") or project.get("active_snapshot_id")
        ...
        payload["retrieval_snapshot_id"] = project.get("active_retrieval_snapshot_id") or project.get("active_snapshot_id")
        payload["indexed_commit"] = project.get("indexed_commit")
        return payload


def apply_generated_synopsis(project_id: str, *, text: str, evidence_ids: list[str], model_id: str,
                             fact_ids: list[str] | None = None,
                             prompt_version: str = INTERPRETATION_PROMPT_VERSION) -> dict:
    ...
    with connect() as conn:
        ...
        row = conn.execute("SELECT id, interpretation_json FROM project_intelligence_snapshots WHERE project_id=? AND owning_snapshot_id=?",
                           (project_id, owning)).fetchone()
        if row is None: raise ValueError("Structured overview must be built before generated prose.")
        available = {item["id"] for item in conn.execute(
            "SELECT id FROM project_intelligence_evidence WHERE intelligence_snapshot_id=?", (row["id"],))}
        ...
        interpretation.update({"generated_synopsis": clean, "generated_evidence_ids": cited,
                               "generated_fact_ids": cited_facts,
                               "generation": {"model_id": str(model_id)[:240], "prompt_version": str(prompt_version)[:120]}})
        conn.execute("UPDATE project_intelligence_snapshots SET interpretation_json=? WHERE id=?",
...
def generate_project_synopsis(project_id: str) -> dict:
    ...
    schema = {
        ...
        "properties": {
            ...
            "fact_ids": {"type": "array", "items": {"type": "string", "enum": sorted(facts)}, "minItems": 1},
    ...
    result = generate_local_structured_json(
        ...
        user_prompt=json.dumps({"facts": [{"fact_id": key, "value": value} for key, value in facts.items()],
                                "available_evidence_ids": evidence_ids}, ensure_ascii=False),
        max_tokens=420,
        json_schema=schema,
    )
    try:
...
def _overview_fact_catalog(snapshot: dict) -> dict[str, object]:
    ...
    if identity.get("purpose"): facts["identity.purpose"] = identity["purpose"]
    technologies = identity.get("technologies") or []
    if technologies: facts["identity.technologies"] = [item.get("name") for item in technologies[:5]]
    for key in ("indexed_file_count", "workspace_count", "symbol_count", "relationship_count", "route_count",
                "community_count", "cycle_count", "execution_flow_count"):
    ...
    if entrypoints: facts["architecture.entrypoints"] = entrypoints[:8]
    return facts
...
def _purpose_candidates(
    *,
    ...
    for relative_path in _PURPOSE_PATHS:
        ...
        evidence_id = "intelligence-evidence-" + hashlib.sha256(
        ...
        evidence.append(
            {
                ...
                "end_line": end_line,
                "extraction_method": "readme_paragraph" if source_type == "readme" else "manifest_description",
    ...
    candidates.sort(key=lambda item: (-int(item["authority"]), str(item["relative_path"]).casefold()))
...
def _manifest_purpose(text: str, path: str) -> tuple[str, int, int, str, str] | None:
    if path == "package.json":
        ...
        if not description:
            return None
        line = next((index for index, raw in enumerate(text.splitlines(), start=1) if '"description"' in raw), 1)
...
def _credible_purpose(value: str) -> bool:
    ...
    if lowered.startswith(("installation", "install ", "quick start", "quickstart", "license", "contributing")):
        return False
...
def _fallback_synopsis(*, name: str, repository_kind: str, file_count: int, technologies: list[dict]) -> str:
...
def _layer(status: str, version: str, generated_at: str | None, unknown_reason: dict | None = None) -> dict:
    return {
        "status": status,
        "version": version,
...
def _legacy_fallback(project: dict) -> dict:
    ...
    layers = {
        ...
        for name in ("identity", "architecture", "repository_signals", "decisions", "interpretation")
    }
    return {
        "id": None,
        "contract_version": INTELLIGENCE_CONTRACT_VERSION,
        "project_id": project["id"],
        "owning_snapshot_id": project.get("active_manifest_snapshot_id") or project.get("active_snapshot_id"),
        "structure_snapshot_id": project.get("active_structure_snapshot_id") or project.get("active_snapshot_id"),
        "retrieval_snapshot_id": project.get("active_retrieval_snapshot_id") or project.get("active_snapshot_id"),
        ...
        "repository_signals": {},
        "decisions": {},
        "interpretation": {"deterministic_synopsis": project.get("brief") or "", "generated_synopsis": None, "primary_evidence_ids": []},
...
def _public_evidence(item: dict) -> dict:
    return {
        key: item.get(key)
        for key in (
            "id", "source_type", "source_id", "relative_path", "source_snapshot",
            "start_line", "end_line", "extraction_method", "extractor_version",
            "confidence_class", "excerpt_hash", "verification_state", "label",
        )
    }
