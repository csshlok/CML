...
from backend.app.core.project_git_intelligence import (
    get_project_repository_state,
    refresh_git_intelligence,
)
from backend.app.core.project_graph import find_nodes, graph_view
from backend.app.core.project_graph_intelligence import (
    get_graph_intelligence,
    refresh_graph_intelligence,
)
...
_STATE_PATTERNS = (
    re.compile(
        r"\b(git|branch|head|working tree|worktree|uncommitted|staged|unstaged|dirty|status)\b",
        re.I,
    ),
...
def run_project_operation(
    ...
    *,
    ...
    target_values = list(
        dict.fromkeys([value.strip() for value in (targets or []) if value.strip()])
    )
    if operation == "blast_radius" and len(target_values) > 1:
        data = {
            "targets": [
                run_project_operation(project_id, operation, target=value, compact=compact)["data"]
        ...
        data = {
            "synopsis": intelligence["interpretation"].get("deterministic_synopsis"),
            ...
            "key_areas": graph.get("communities", [])[:6],
            "execution_flows": graph.get("flows", [])[:5],
            "evidence": intelligence.get("evidence", [])[:8],
        }
    elif operation == "project_state":
        data = get_project_repository_state(project_id)
        ...
        paths = list(
            ...
            or [item["relative_path"] for item in state.get("live_state", {}).get("files", [])]
        )
        data = {
            "changes": changes,
            "repository_state": state,
            "files": _change_details(project_id, paths),
        ...
        if not target:
            raise ValueError("blast_radius requires a target symbol or path.")
        candidates = find_nodes(project_id, target, limit=8)
        if not candidates:
            data = {"status": "not_found", "target": target, "candidates": []}
        elif len(candidates) > 1 and not any(
            target == value.get("qualified_id") for value in candidates
        ):
            data = {"status": "ambiguous", "target": target, "candidates": candidates}
        else:
            chosen = next(
                (item for item in candidates if target == item.get("qualified_id")), candidates[0]
            )
            view = graph_view(
                project_id,
                mode="graph",
                root=chosen["qualified_id"],
                query=target,
                direction="inbound",
                max_depth=3,
                max_nodes=160,
            ...
            impacted_paths = sorted(
                {node.get("relative_path") for node in view["nodes"] if node.get("relative_path")}
            )
            data = {
                "status": "found",
                "target": chosen,
        ...
        data = list_project_decisions(project_id)
    elif operation == "coverage":
        data = (
            calculate_test_impact(
                project_id, changed_paths=changed_paths or [], changed_lines=changed_lines
            )
            if changed_paths
            else get_project_coverage(project_id)
        )
    else:
        candidates = []
        ...
        data = {
            "query": query,
            "structural_hits": list(unique.values())[:12],
    ...
    return {
        ...
        "operation": operation,
        "display": "compact" if compact else "expanded",
        "data": data,
    }
...
def refresh_project_intelligence_layers(project_id: str) -> dict:
    ...
    for name, callback in (
        ("graph", refresh_graph_intelligence),
        ("git", refresh_git_intelligence),
        ("decisions", refresh_project_decisions),
    ):
        try:
            outcomes[name] = {"status": "ready", "result": callback(project_id)}
...
def enqueue_project_intelligence_layers(
    project_id: str,
    *,
    ...
    mapping = {
        "overview": "project_intelligence_overview",
        "graph": "project_graph_metrics",
        "git": "project_git_intelligence",
        "decisions": "project_decision_refresh",
    ...
    if unknown:
        raise ValueError(f"Unsupported intelligence layer: {unknown[0]}")
...
def _change_details(project_id: str, paths: list[str]) -> list[dict]:
    ...
    with connect() as conn:
        project = conn.execute(
            "SELECT active_structure_snapshot_id, active_snapshot_id FROM projects WHERE id=?",
            (project_id,),
        ).fetchone()
        snapshot = project["active_structure_snapshot_id"] or project["active_snapshot_id"]
        git = conn.execute(
            "SELECT id FROM project_git_snapshots WHERE project_id=? ORDER BY generated_at DESC LIMIT 1",
            (project_id,),
        ).fetchone()
        items = []
        for path in normalized:
            ...
            communities = sorted(
                {row["community_label"] for row in nodes if row["community_label"]}
            )
            reasons = ["Changed in the current repository state"]
            if dependents:
                reasons.append(f"{len(dependents)} structurally dependent file(s)")
            if cochange:
                reasons.append(f"{len(cochange)} historical co-change partner(s), not dependencies")
            items.append(
                {
                    "relative_path": path,
                    "why_included": reasons,
                    "communities": communities,
                    "structural_dependents": dependents,
...
def _change_unknowns(state: dict) -> list[str]:
    ...
    if state.get("status") != "ready":
        unknowns.append("Git history is unavailable.")
    if state.get("history_truncated"):
        unknowns.append("Git history was bounded; older co-change evidence is not included.")
    if state.get("shallow_history"):
        unknowns.append("This is a shallow clone, so history-derived signals are incomplete.")
    return unknowns
