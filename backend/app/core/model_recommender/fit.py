...
def estimate_expert_fit(profile: dict[str, Any], candidate: dict[str, Any]) -> dict[str, Any]:
    family = str(candidate.get("family") or "")
    accepted = bool((candidate.get("compatibility") or {}).get("expert_role_accepted"))
    local_path = str(candidate.get("local_path") or "")
    total_b = float(candidate.get("parameter_count_total_b") or candidate.get("parameter_count_total") or 0.0)
    runtime_required = gib_to_bytes(max(4.0, total_b * 2.2))
    training_required = gib_to_bytes(max(8.0, total_b * 3.0))
    usable_ram = int(profile.get("ram_usable_bytes") or 0)
    training_supported = bool(profile.get("training_supported"))
    tier = str(profile.get("hardware_tier") or "unknown")
    warnings: list[str] = []
    runtime_fit_type = "cannot_run"
    runtime_feasible = False
    if accepted and local_path and usable_ram >= runtime_required:
        runtime_fit_type = "cpu_local_runtime"
        runtime_feasible = True
    elif accepted and local_path:
        warnings.append("The checkpoint is accepted, but this device does not have enough conservative RAM headroom for expert runtime.")
    training_fit_type = "blocked"
    training_feasible = False
    if accepted and local_path and training_supported and usable_ram >= training_required and minimum_tier_satisfied(tier, str(candidate.get("minimum_expert_tier") or "cpu_minimum_spec")):
        training_fit_type = "local_training"
        training_feasible = True
    else:
        if not training_supported:
            warnings.append("Expert training remains blocked because the device has not passed the AVX2 and hardware support gate.")
        elif not local_path:
            warnings.append("Expert training needs an installed local checkpoint, not only a managed GGUF chat runtime.")
    return {
        "family": family,
        "runtime_fit_type": runtime_fit_type,
        "runtime_feasible": runtime_feasible,
        "training_fit_type": training_fit_type,
        "training_feasible": training_feasible,
        "runtime_required_gib": round(runtime_required / 1024**3, 2),
        "training_required_gib": round(training_required / 1024**3, 2),
        "warnings": warnings,
    }
